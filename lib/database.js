...
export class Database {
  ...
  static async saveArticle(userId, articleData) {
    ...
    // Check for duplicate URL (shouldn't happen with new logic, but keep as safety)
    ...
    const article = {
      ...
      updated_at: new Date().toISOString(),
      lastHighlighted: articleData.lastHighlighted || null
    };
  ...
  // NEW METHOD: Update existing article (for highlights)
  static async updateArticle(articleId, updates) {
    const article = await this.getArticle(articleId);
    if (!article) {
      console.error('Article not found for update:', articleId);
      return null;
    }
    ...
    // Update the user's articles list with new highlight count
    const userArticles = await this.getUserArticles(article.user_id);
    const updatedUserArticles = userArticles.map(userArticle => {
    ...
    await kv.set(KEYS.userArticles(article.user_id), updatedUserArticles, { ex: CONFIG.ARTICLE_CACHE_TTL });
    
    console.log('✅ Article updated successfully:', articleId, 'highlights:', updatedArticle.highlights?.length || 0);
